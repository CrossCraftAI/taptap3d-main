...
| D6 | **Templates as data** — a validated layout schema rather than hardcoded shapes | Larger than it sounds: templates currently declare only `{perPage, imagePlacement}`, and anything outside that vocabulary needs new geometry in the layout engine | M1; a decision on how much structure the engine keeps | Lets houses and, later, the AI author layouts. Without it, every new layout is an engineering ticket |
