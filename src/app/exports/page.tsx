 ...
 * today", and until this page the only way to reach it was through the editor.
 * Someone sending last week's settled catalogue to the printer should not have
 * to open the editor to do it — that is the difference between composing and
 * publishing, and it is why they are different places on the rail.
 *
 * The route is `/exports` and the heading says "Exports", though the PDF is the
 * only output that exists: the tearsheet and the price list landed as TEMPLATES
 * (ROADMAP D6) rather than as outputs of their own, so the catalogue screen
 * chooses one and this same link prints whichever is chosen, through the same
 * renderer. The listing and the per-lot embed are D2–D4; when one lands it is a
 * column here, not a new place, and nothing has to be renamed.
 *
